...
module conv2d_top #(
  ...
  wire compute_done;
  assign compute_done = (h_cnt==H_OUT-1) && (w_cnt==W_OUT-1) && (oc_cnt==C_OUT-1);
  
  reg signed [7:0] weight_buf[0:215];
  reg signed [7:0] act_buf[0:3071];
  reg signed [31:0] bias_buf[0:7];
  reg signed [7:0] out_buf[0:7199];
  ...
  always @(posedge clk) begin
    if(rst) begin
      ...
      done <= (state==OUTPUT) && (next_state==DONE_S);
      busy <= (state==COMPUTE);
    end
  end
  
  // AXI4-Lite write and read control
  always @(posedge clk) begin
    if(rst) begin
      reg_start <= 0;
  ...
  // Activation and weight buffer indexing
  ...
  assign mac_b0 = weight_buf[oc_cnt*9+0];
  assign mac_b1 = weight_buf[oc_cnt*9+1];
  assign mac_b2 = weight_buf[oc_cnt*9+2];
  assign mac_b3 = weight_buf[oc_cnt*9+3];
  assign mac_b4 = weight_buf[oc_cnt*9+4];
  assign mac_b5 = weight_buf[oc_cnt*9+5];
  assign mac_b6 = weight_buf[oc_cnt*9+6];
  assign mac_b7 = weight_buf[oc_cnt*9+7];
  assign mac_b8 = weight_buf[oc_cnt*9+8];
  ...
  mac_array u_mac(
    ...
    .a0(mac_a0), .a1(mac_a1), .a2(mac_a2), .a3(mac_a3), .a4(mac_a4),
    .a5(mac_a5), .a6(mac_a6), .a7(mac_a7), .a8(mac_a8),
    .b0(mac_b0), .b1(mac_b1), .b2(mac_b2), .b3(mac_b3), .b4(mac_b4),
    .b5(mac_b5), .b6(mac_b6), .b7(mac_b7), .b8(mac_b8),
    .acc_out(mac_out)
  );
...
// ============================================================================
// MAC Array — 9 parallel 32-bit multipliers with accumulation
// ============================================================================
...
module mac_array(
  ...
  // 32x32 signed multipliers (sign-extended from 8-bit inputs)
...
// ============================================================================
// Requantizer — Arithmetic right shift and saturation to INT8
// ============================================================================
